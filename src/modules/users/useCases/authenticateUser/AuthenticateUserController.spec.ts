...
describe("Authenticate User", () => {
  beforeAll(async () => {
    ...
    userGlobal = await connection.query(
  ...
  it("Should not be able to create a session with wrong email", async () => {
    ...
    expect(authenticationResponse.text)
    .toEqual('{"message":"Incorrect email or password"}')
  })
  it("Should not be able to create a session with wrong password", async () => {
    ...
    expect(authenticationResponse.status).toEqual(401)
    expect(authenticationResponse.text).toEqual('{"message":"Incorrect email or password"}')
  })
})
